...
//This code doesn't seem to work? I don't want the user to have to pass in the background color so this is just gonna remain returning false for now. Maybe it has something to do with the "readPixel" function? the commented out section is the code as it would've been.
bool InternalUtils::isLightBackground(Adafruit_TFTLCD &gfx, int x, int y){
    //uint16_t color = gfx.readPixel(x, y);
    //return color > 0x7FFF;

    return TFTGraph::drawForLightBackground;
}